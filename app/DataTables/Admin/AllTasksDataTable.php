...
class AllTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('approve', function ($row) use ($taskBoardColumns) {
            $approve = '<button type="submit" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#exampleModalCenter">
                        <i class="fa fa-edit"></i>
                      </button>'.$row->approve .' <br/> '.$row->reasonas.'';
                    $approve .='<div class="modal fade bs-modal-md in" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">';
                    $approve .='<div class="modal-dialog modal-dialog-centered" role="document">';
                        $approve .='<div class="modal-content">';
                            $approve .='<form method="post" action="'.url('admin/task/approve_or_reject_requisition') .'/' . $row->id.'">';
                            $approve .= method_field('put');
                            $approve .=csrf_field();
                            $approve .='<div class="modal-body">';
                               $approve .='<div class="form-group">';
                                    $approve .='<label style="color: black">اختيار حالة الطلب</label>';
                                    $approve .='<select  class="form-control" name ="approve" required  style="color: black;margin:3px;" id="req_status">';
                                    $approve .='<option disabled selected value="">اختيار حالة الطلب</option>';
                                    $approve .='<option value="انتظار">انتظار</option>';
                                    $approve .='<option value="قبول">قبول</option>';
                                    $approve .='<option value="رفض">رفض</option>';
                                    $approve .='</select>';
                                $approve .='</div>';
                                $approve .='<div class="form-group" id="reason_div">';
                                    $approve .='<label style="color: black;margin:3px;">سبب الرفض</label>';
                                    $approve .='<input type="text" class="form-control" placeholder="سبب الرفض" name="reasonas">';
                                $approve .='</div>';
                            $approve .='</div>';
                            $approve .='<div class="modal-footer">';
                                $approve .='<button type="button" class="btn btn-secondary" data-dismiss="modal">الغاء</button>';
                                $approve .='<button type="submit" class="btn btn-primary">حفظ</button>';
                            $approve .='</div>';
                                $approve .='</form>';
                        $approve .='</div>';
                    $approve .='</div>';
            ...
            ->addColumn('approve', function ($row) use ($taskBoardColumns) {
              return ucfirst($row->column_name);
            })
            ->editColumn('reasonas', function ($row){
              return 'kkk';
            })
            ...
            ->rawColumns(['board_column', 'action','approve','reasonas','project_name', 'clientName', 'due_date', 'users', 'created_by', 'heading'])
    ...
    public function query(Task $model)
    {
        ...
        $model = $model->leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
            ...
            ->selectRaw('tasks.id, tasks.approve,tasks.reasonas,projects.project_name, tasks.heading, client.name as clientName, creator_user.name as created_by, creator_user.image as created_image,
             tasks.due_date,tasks.start_date, taskboard_columns.column_name as board_column, taskboard_columns.label_color,
        ...
        if ($request->approve != '' && $request->approve !=  null && $request->approve !=  'all') {
            $model->where('tasks.approve', '=', $request->approve);
        }
        if ($request->reasonas != '' && $request->reasonas !=  null && $request->reasonas !=  'all') {
            $model->where('tasks.reasonas', '=', $request->reasonas);
        }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.approve') => ['data' => 'approve', 'name' => 'approve', 'visible' => true],
