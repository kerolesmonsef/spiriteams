 ...
 * @property int $id
 * @property EmployeeDetails $employeeDetail
 */
class User extends BaseModel implements AuthenticatableContract, AuthorizableContract, CanResetPasswordContract
{
    ...
    {
        return self::query()->whereNotNull('uuid')->where("uuid", $uuid)->first();
    }
    ...
    {
        /** @var Role[] $roles */
        $roles = $this->roles->load("p_permissions");

        $permissions = collect();

        foreach ($roles as $role) {
            $permissions = $permissions->merge($role->p_permissions);
        }
