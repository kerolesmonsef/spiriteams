...
class MemberAllTasksController extends MemberBaseController
{
    ...
    public function data(Request $request, $startDate = null, $endDate = null, $hideCompleted = null, $projectId = null)
    {
        ...
        return DataTables::of($tasks)
            ...
            ->editColumn('start_date', function ($row) {
                return '<span >' . $row->start_date->format($this->global->date_format) . '</span>';
            })
            ...
            ->editColumn('approve', function ($row) {
                return '<span >' . $row->approve . '</span>';
            })
            ->editColumn('reasonas', function ($row) {
                return '<span >' . $row->reasonas . '</span>';
            })
            ...
            ->rawColumns(['board_column', 'action', 'project_name', 'created_by', 'start_date', 'due_date', 'users', 'heading'])
    ...
    public function edit($id)
    {
        ...
        if (!$this->task->is_task_user || !$this->task->users->contains(auth()->user())) {
            abort(403);
        }
