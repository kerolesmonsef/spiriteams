...
use App\DataTables\Admin\RefundInvoicesDataTable;
use App\Estimate;
...
use App\Http\Requests\Invoices\RefundInvoice;
use App\Invoice;
...
class ManageAllInvoicesController extends AdminBaseController
{
    ...
    public function destroy($id)
    {
        ...
        if ($firstInvoice->id == $id) {
            ...
            Invoice::destroy($id);
            $this->logUserActivity($this->user->id,__('messages.invoiceDeleted'));
    ...
    public function store(StoreInvoice $request)
    {
        ...
        $this->logUserActivity($this->user->id,__('messages.invoiceCreated'));
        return Reply::redirect(route('admin.all-invoices.index'), __('messages.invoiceCreated'));
    }

    public function refunds(RefundInvoicesDataTable $dataTable)
    {
        if(!request()->ajax()) {
            $this->projects = Project::allProjects();
            $this->clients  = User::allClients();
        }
        $this->pageTitle = 'app.refunds';
        return $dataTable->render('admin.invoices.refunds', $this->data);
    }
    ...
    public function addRefundInvoice(RefundInvoice $request,$id)
    {
        $items         = $request->input('item_name');
        $itemsSummary  = $request->input('item_summary');
        $cost_per_item = $request->input('cost_per_item');
        ...
        $invoice->currency_id = $request->currency_id;
        $invoice->recurring = 'no';
        $invoice->billing_frequency = $invoiceInfo->billing_frequency;
        $invoice->billing_interval = $invoiceInfo->billing_interval;
        $invoice->billing_cycle = $invoiceInfo->billing_cycle;
        $invoice->note = $request->note;
        $invoice->invoice_refund = 'yes';
        $invoice->invoice_refund_id = $id;
        $invoice->save();

        Invoice::where('id',$id)->update(['invoice_refund_id' => $invoice->id ]);
        $this->logUserActivity($this->user->id,__('messages.invoiceCreated'));
        return Reply::redirect(route('admin.all-invoices.refunds'), __('messages.invoiceCreated'));
    }
    ...
    public function update(StoreInvoice $request, $id)
    {
        ...
        $this->logUserActivity($this->user->id,__('messages.invoiceUpdated'));
        return Reply::redirect(route('admin.all-invoices.index'), __('messages.invoiceUpdated'));
    }

    public function show($id)
    {
       // $this->project_number = Project::where('id', $this->invoice->project_id)->with('clientdetails')->first();
        $this->invoice = Invoice::findOrFail($id)->withCustomFields();
        $this->fields = $this->invoice->getCustomFieldGroupsWithFields()->fields;
        $this->paidAmount = $this->invoice->getPaidAmount();
    ...
    public function deleteAppliedCredit(Request $request, $id)
    {
        ...
        if ($this->creditNotes->count() > 0) {
            ...
            $this->logUserActivity($this->user->id,__('messages.creditedInvoiceDeletedSuccessfully'));
    ...
    public function storeFile(InvoiceFileStore $request)
    {
        ...
        if ($invoice != null) {
            ...
            $invoice->save();
            $this->logUserActivity($this->user->id,__('messages.fileUploadedSuccessfully'));
    ...
    public function sendInvoice($invoiceID)
    {
       ...
       //  }
        ...
        $invoice->save();
        // // $this->logUserActivity($this->user->id,__('messages.updateSuccess'));
