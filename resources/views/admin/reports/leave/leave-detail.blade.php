...
<div class="modal-header">
    ...
    <h4 class="modal-title"
        id="myLargeModalLabel">@if($modalHeader == 'approved') @lang('app.approved') @elseif($modalHeader == 'pending') @lang('app.pending') @else @lang('app.upcoming') @endif @lang('app.menu.leaves') @lang('app.details')</h4>
</div>
            ...
            <div class="white-box">
                @foreach($leave_types as $leave_type)
                    <div class="col-md-4 text-center">
                        <h4><span class="text-{{ $leave_type->color }}">{{ $leave_type->leaves->count() }}</span> <span
                                    class="font-12 text-muted m-l-5"> {{ $leave_type->type_name }}</span></h4>
                    </div>
                @endforeach
            </div>
        </div>
