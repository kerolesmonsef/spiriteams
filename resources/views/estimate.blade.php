                ...
                <div class="col-md-offset-2 col-md-8 col-md-offset-2" id="estimates">
                    <div class="row m-b-20">
                        <div class="col-md-12">
                            ...
                            @endif
                            <a href="{{ route("front.downloads", md5($estimate->id)) }}" target="_blank" class="btn btn-default pull-right m-r-10"><span><i class="fa fa-file-pdf-o"></i> @lang('modules.invoices.downloadPdf')</span></a>
                            <div class="clearfix"></div>
                        </div>
                    </div>
                    <div class="card">
                                                        ...
                                                        <div class="col-md-3">
                                                            ...
                                                            <p class="text-muted">
                                                                ...
                                                                @elseif($field->type == 'select')
                                                                    {{ (!is_null($estimate->custom_fields_data['field_'.$field->id]) && $estimate->custom_fields_data['field_'.$field->id] != '') ? $field->values[$project->custom_fields_data['field_'.$field->id]] : '-' }}
                                                                @elseif($field->type == 'checkbox')
                                                                <ul>
                                                                    @foreach($field->values as $key => $value)
                                                                        @if($estimate->custom_fields_data['field_'.$field->id] != '' && in_array($value ,explode(', ', $estimate->custom_fields_data['field_'.$field->id]))) <li>{{$value}}</li> @endif
                                                                    @endforeach
                                                                </ul>
                                                                @elseif($field->type == 'date')
                                                                    {{ !is_null($estimate->custom_fields_data['field_'.$field->id]) ? \Carbon\Carbon::parse($estimate->custom_fields_data['field_'.$field->id])->format($global->date_format) : '--'}}
                                                                @endif
                                                            </p>
