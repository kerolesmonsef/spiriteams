...
@section('content')
                                    ...
                                    <div class="white-box">
                                        ...
                                        <ul class="list-group" id="invoices-list">
                                            ...
                                            @forelse($project->invoices as $invoice)
                                                ...
                                                @if ($invoice != 'draft' && $invoice->send_status)
                                                    <li class="list-group-item">
                                                        <div class="row">
                                                            <div class="col-md-5 col-xs-12">
                                                                <a style="text-decoration: underline"
                                                                    href="{{ route('client.invoices.show', $invoice->id) }}">{{ $invoice->invoice_number }}</a>
                                                            </div>
                                                            ...
                                                            <div class="col-md-2 col-xs-6">
                                                                @if ($invoice->status == 'unpaid')
                                                                    <label
                                                                        class="label label-danger">{{ strtoupper($invoice->status) }}</label>
                                                                @else
                                                                    <label
                                                                        class="label label-success">{{ strtoupper($invoice->status) }}</label>
                                                                @endif
                                                            </div>
                                                            <div class="col-md-3">
                                                                <a href="{{ route('client.project-invoice.download', $invoice->id) }}"
                                                                    data-toggle="tooltip" data-original-title="Download"
                                                                    class="btn btn-inverse btn-circle"><i
                                                                        class="fa fa-download"></i></a>
                                                                <span
                                                                    class="m-l-10">{{ $invoice->issue_date->format($global->date_format) }}</span>
                                                            </div>
                                                        </div>
                                                    </li>
                                                    @php
    ...
    {{-- Ajax Modal --}}
    <div class="modal fade bs-modal-lg in" id="add-invoice-modal" role="dialog" aria-labelledby="myModalLabel"
        aria-hidden="true">
